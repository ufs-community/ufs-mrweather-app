...
Among other techniques, MET provides the capability to compute standard verification scores for comparing deterministic gridded model data to point-based and gridded observations. It also provides ensemble and probabilistic verification methods for comparing gridded model data to point-based or gridded observations. Currently, the ``EMC_verif-global`` subcomponent of the MRW App's Global Workflow supports the use of :term:`GDAS` and :term:`NAM` observation files in prepBUFR format for point-based (grid-to-observation) verification. Recent data files are publicly available for `GDAS <https://nomads.ncep.noaa.gov/pub/data/nccf/com/gfs/prod/>`__ and `NAM <https://nomads.ncep.noaa.gov/pub/data/nccf/com/nam/prod/>`__ on the :term:`NOMADS` website. ``EMC_verif-global`` also supports use of gridded Climatology-Calibrated Precipitation Analysis (:term:`CCPA`) 24-hour accumulation data for accumulated precipitation evaluation, and it uses the model's own analysis file for grid-based (grid-to-grid) verification. 
