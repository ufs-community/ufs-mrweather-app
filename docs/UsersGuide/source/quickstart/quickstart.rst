...
The following quick start guide is for versions of the mrweather app
that have already been ported to the local target machine. The
workflow for building and running mrweather is built on the CIME
(Common Infrastructure for Modeling Earth) framework.  Please refer to
the `CIME Porting Documentation
<http://esmci.github.io/cime/users_guide/porting-cime.html>`_ if CIME
...
This is the procedure for quickly setting up and running a mrweather case.

Download the mrweather app (see `Downloading CESM2 <downloading_cesm.html>`_).
